export const DEFAULT_SITE_SETTINGS = {
  scope: "main",
  company_name: "엔드아이에셋대부",
  company_subtitle: "주택담보대출 · 대환대출 · 전세퇴거자금 상담",
  logo_url: "/andi-logo.jpg",
  phone: "070-8018-7437",
  kakao_id: "ANDi7437",
  kakao_url: "https://open.kakao.com/o/sbaltXmi",
  hero_badge: "선택형 시세조회 · 빠른 상담 연결",
  hero_title: `아파트 시세조회부터
대출 상담 신청까지
한 번에 연결되는 구조`,
  hero_description: "시세조회와 상담 신청을 한 화면에서 간편하게 이용할 수 있습니다.",
  hero_feature_1: "무료 한도 상담",
  hero_feature_2: "빠른 접수 확인",
  hero_feature_3: "맞춤 상담 연결",
  hero_primary_cta: "빠른 시세조회",
  hero_secondary_cta: "무료 상담 신청",
  consult_button_text: "상담 신청",
  reviews_enabled: true,
  hero_background_url: "",
  notice_enabled: false,
  notice_text: "금리와 한도는 조건에 따라 달라질 수 있으니 상담을 통해 정확하게 안내해드립니다.",
  popup_enabled: false,
  popup_title: "빠른 상담 안내",
  popup_description: "간편접수 또는 대표 상담 채널로 편하게 문의해 주세요.",
  popup_button_text: "상담 바로가기",
  popup_button_url: "#contact",
  middle_banner_enabled: false,
  middle_banner_badge: "맞춤 상담 안내",
  middle_banner_title: "조건에 맞는 상담 연결을 빠르게 도와드립니다.",
  middle_banner_description: "상담 목적에 맞는 안내를 빠르게 도와드립니다.",
  middle_banner_button_text: "상담 문의하기",
  middle_banner_button_url: "#contact",
  representative_name: "최종원",
  business_registration_number: "739-08-03168",
  brokerage_registration_number: "2025-서울서초-0084",
  lending_registration_number: "2025-서울서초-0083(대부업)",
  company_address: "서울특별시 서초구 서초중앙로 114, 일광빌딩 지하2층 B204호",
  registration_agency: "서초구청 일자리경제과 (02-2155-8752)",
  footer_legal_line_1: "이자율 : 연6% ~ 연20%이내 (연체이자율 연 7% ~ 20% 이내, 취급수수료 및 기타 부대비용없음)",
  footer_legal_line_2: "중개수수료를 요구하거나 받는 것은 불법입니다.",
  footer_legal_line_3: "과도한 빚, 고통의 시작입니다. 대출시 귀하의 신용등급이 하락할 수 있습니다.",
  footer_legal_line_4: "이 사이트에서 광고되는 상품들의 상환 기간은 모두 60일 이상이며 (최저 2개월, 최대 5년), 최대 연 이자율은 20%입니다.",
  footer_legal_line_5: "대부이자율 (연 이자율) 및 연체이자율은 연 20%를 초과할 수 없습니다. (조기상환 조건없음)",
  footer_copyright: "© 엔드아이에셋대부. All Rights Reserved.",
};

const BOOLEAN_FIELDS = new Set(["reviews_enabled", "notice_enabled", "popup_enabled", "middle_banner_enabled"]);

export function normalizeSiteSettings(row = {}) {
  const normalized = {
    ...DEFAULT_SITE_SETTINGS,
    ...Object.fromEntries(Object.entries(row || {}).filter(([, value]) => value !== null && value !== undefined)),
  };

  for (const key of BOOLEAN_FIELDS) {
    normalized[key] = parseBoolean(normalized[key], DEFAULT_SITE_SETTINGS[key]);
  }

  return normalized;
}

export function parseBoolean(value, fallback = false) {
  if (typeof value === "boolean") return value;
  if (typeof value === "number") return value === 1;
  if (typeof value === "string") {
    const lower = value.trim().toLowerCase();
    if (["true", "1", "yes", "y", "on"].includes(lower)) return true;
    if (["false", "0", "no", "n", "off"].includes(lower)) return false;
  }
  return fallback;
}


export function getSiteSettingsCacheKey(scope = "main") {
  return `loanLanding.siteSettings.${scope || "main"}`;
}

export function readCachedSiteSettings(scope = "main") {
  if (typeof window === "undefined") return DEFAULT_SITE_SETTINGS;
  try {
    const raw = window.localStorage.getItem(getSiteSettingsCacheKey(scope));
    if (!raw) return DEFAULT_SITE_SETTINGS;
    return normalizeSiteSettings(JSON.parse(raw));
  } catch {
    return DEFAULT_SITE_SETTINGS;
  }
}

export function cacheSiteSettings(settings, scope = settings?.scope || "main") {
  if (typeof window === "undefined") return;
  try {
    window.localStorage.setItem(getSiteSettingsCacheKey(scope), JSON.stringify(normalizeSiteSettings(settings)));
  } catch {}
}
