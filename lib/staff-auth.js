...
export function normalizeStaffRole(role) {
  ...
  if (["admin", "representative", "marketing", "cs"].includes(normalized)) return normalized;
  if (["owner", "ceo", "대표", "대표자"].includes(normalized)) return "representative";
  if (["manager", "관리자"].includes(normalized)) return "admin";
  if (["marketer", "marketing_manager", "마케팅", "마케팅담당"].includes(normalized)) return "marketing";
  if (["customer", "counselor", "worker", "staff", "상담", "cs담당", "실무자"].includes(normalized)) return "cs";
  return "cs";
}

export function getStaffRoleLabel(role) {
  return ROLE_LABELS[normalizeStaffRole(role)] || "CS담당";
}

export function canAccessOwnerConsole(account) {
  const role = normalizeStaffRole(account?.role);
  return role === "admin" || role === "representative";
}

export function canManageSite(account) {
  const role = normalizeStaffRole(account?.role);
  return role === "admin" || role === "representative" || role === "marketing";
}

export function canAccessStaffCRM(account) {
  const role = normalizeStaffRole(account?.role);
  return role === "admin" || role === "representative" || role === "cs";
}

export function getAllowedPages(account) {
  const role = normalizeStaffRole(account?.role);
  if (role === "admin" || role === "representative") return ["/admin", "/staff", "/manage", "/manage-mobile"];
  if (role === "marketing") return ["/manage", "/manage-mobile"];
  return ["/staff"];
}
