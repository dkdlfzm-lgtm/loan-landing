...
export async function GET(_request, { params }) {
  ...
  try {
    const notes = await supabaseRest('/inquiry_notes', {
      query: {
        select: 'id,inquiry_id,author,content,created_at',
        inquiry_id: `eq.${params.id}`,
...
export async function POST(request, { params }) {
  ...
  try {
    ...
    if (!author || !content) {
      return NextResponse.json({ ok: false, message: '작성자와 내용을 입력해주세요.' }, { status: 400 });
    }

    const inserted = await supabaseRest('/inquiry_notes', {
      method: 'POST',
      prefer: 'return=representation',
      body: [{ inquiry_id: params.id, author, content }],
    });

    await supabaseRest(`/inquiries?id=eq.${params.id}`, {
      method: 'PATCH',
      body: { call_summary: content.slice(0, 180), updated_at: new Date().toISOString() },
    });
