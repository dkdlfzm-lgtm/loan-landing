...
function getTradeApartmentName(item) {
  return normalizeText(item.aptNm || item.아파트 || item.offiNm || item.단지 || item.apartmentName || "");
}
...
function pickCatalogEntry(master, query) {
  ...
  const best = exact.find((row) => (row.areas || []).some((area) => Math.abs(parseAreaNumber(area) - targetArea) <= toleranceForArea(targetArea)));
  return best || exact[0];
}
...
async function fetchRealTradeSummary(query) {
  ...
  const matched = responses
    ...
    .filter((row) => {
      ...
      const areaOk = !Number.isFinite(targetArea) || Math.abs(Number(row.area || 0) - targetArea) <= maxAreaDiff;
  ...
  return {
    ...
    summary: {
      ...
      description: `최근 ${matched.length}건의 실거래 신고 자료를 기준으로 계산한 결과입니다. 최근 실거래가 ${formatEok(latest.amountWon)}, 평균 ${formatEok(avg)} 수준입니다.`,
...
async function fetchStatSummary(query) {
  ...
  if (!key || !statId) return { ...fallback, warning: "실거래가 API와 통계 API 설정이 모두 없어 결과를 제공하지 못했습니다." };
  ...
  if (!response.ok) return { ...fallback, warning: `R-ONE API 호출 실패 (${response.status})` };
